...
urlAppli = "http://gsb/"     #
urlRenseignerFicheFrais ="http://gsb/index.php?uc=gererFrais&action=saisirFrais"                                     #
...
#valeur fiche de frais
forfaitEtape = 150
fraisKilomètre = 50
nuit = 5
repasRestaurant = 5;
dateFrais = '11/11/2021'
libelle='Floralys'
montant=120               #
#######################################
...
#accées à la page fiche de frais
try:
    driver.get(urlRenseignerFicheFrais)
    log("- Accès à la page Rensigner fiche frais : OK")
except Exception as ex:
    log("- Erreur, problème d'accès à la page fiche frais...")
    print(ex)

#ajout des valeur dans fiche de frais : éléments forfaitisés

try:
    setValeurInput('input[name="lesFrais[ETP]"]', forfaitEtape)
    setValeurInput('input[name="lesFrais[KM]"]', fraisKilomètre)
    setValeurInput('input[name="lesFrais[NUI]"]', nuit)
    setValeurInput('input[name="lesFrais[REP]"]', repasRestaurant)
    driver.find_element(By.TAG_NAME, 'form').submit()
    log("- ajout des valeurs éléments forfaitisés : OK")
except Exception as ex:
    log("- Erreur dans l'ajout des valeurs forfaitisés'...")
    print(ex)

#ajout des valeur dans fiche de frais : Nouvel élément élément hors forfait

try:
    setValeurInput('input[name="dateFrais"]', dateFrais)
    setValeurInput('input[name="libelle"]', libelle)
    setValeurInput('input[name="montant"]', montant)
    driver.find_element(By.TAG_NAME, 'BoutonValide').submit()
